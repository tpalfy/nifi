...
public class SNMPV3Configurations implements SNMPConfigurations {
    ...
    public SNMPConfiguration createSnmpListenTrapConfig(final int managerPort) {
        return SNMPConfiguration.builder()
                .setManagerPort(String.valueOf(managerPort))
                .setVersion(SnmpConstants.version3)
                .build();
    }

    public SNMPConfiguration createSnmpGetSetConfigWithCustomAuth(final int agentPort, final String securityName, final String authPassphrase) {
        return SNMPConfiguration.builder()
                .setTargetHost(DEFAULT_HOST)
                .setTargetPort(String.valueOf(agentPort))
                .setCommunityString(COMMUNITY_STRING)
